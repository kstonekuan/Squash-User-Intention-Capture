...
export async function testAIModel(): Promise<{
  ...
  try {
    ...
    try {
      ...
      diagnosticResult.error = `Error checking availability: ${availabilityError instanceof Error ? 
        availabilityError.message : 'Unknown error'}`;
      return diagnosticResult;
    }
    ...
    try {
      ...
      diagnosticResult.error = `Error sending prompt: ${promptError instanceof Error ? 
        promptError.message : 'Unknown error'}`;
    } finally {
      // Clean up session
      if (session) {
        try {
    ...
    diagnosticResult.error = `Unexpected error during test: ${error instanceof Error ? 
 ...
 */
export async function analyzeWorkflow(events: RawEvent[], customPrompt?: string): Promise<WorkflowAnalysis> {
  ...
  try {
    ...
    try {
      ...
      return {
        ...
        steps: [{
          ...
          intent: availabilityError instanceof Error ? availabilityError.message : 'Unknown error'
        }],
        debug: {
          prompt,
    ...
    let result;
    try {
      console.log('Sending prompt to model:', prompt ? (prompt.substring(0, 100) + '...') : 'undefined prompt');
      ...
      return {
        ...
        debug: {
          prompt,
          error: promptError instanceof Error ? 
